...
def parse_args():
    ...
    parser.add_argument('--dataset_name', type=str, default='Istanbul')
    ...
    parser.add_argument('--emb_path', default="../Splitter/output/Istanbul_embedding.csv")
    parser.add_argument('--maps_path', default="../Splitter/output/Istanbul_personas.json")
...
if __name__ == "__main__":
    ...
    def load_emb(path):
        ...
        for line in file:
            if count == 0:
                ...
                continue 
            data_line = line.strip().split(",")
            emb = [float(ele) for ele in data_line[1:]]
            embs.append(emb)
        embs = np.array(embs) 
        return embs
    ...
    maps_PtOri, maps_OritP = load_maps(args.maps_path)
